  ...
  return (
    <section aria-label="KPI cards" className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {kpiItems.map((item) => (
        <article
          key={item.title}
          className="rounded-2xl border border-white/15 bg-white/10 p-5 shadow-xl backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-[#22D3EE]/40"
        >
